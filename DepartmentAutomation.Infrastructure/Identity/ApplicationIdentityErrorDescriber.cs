    ...
    public class ApplicationIdentityErrorDescriber : IdentityErrorDescriber
    {
        ...
        public override IdentityError InvalidUserName(string userName)
        {
            return new IdentityError
            {
                ...
                Description = $"User name '{userName}' is invalid, allowable characters 'a-z 'A-Z'.",
            };
        }
        ...
        {
            return new IdentityError
                {Code = nameof(DuplicateUserName), Description = $"User Name '{userName}' is already taken."};
        }
        
        public IdentityError DuplicateFullName(string name, string surname, string patronymic)
        {
            return new IdentityError
                {Code = nameof(DuplicateFullName), Description = $"User with full name '{name} {surname} {patronymic}' is already taken."};
        }

        public override IdentityError DuplicateEmail(string email)
        {
            return new IdentityError
                {Code = nameof(DuplicateEmail), Description = $"Email '{email}' is already taken."};
        }
        ...
        {
            return new IdentityError
                {Code = nameof(UserAlreadyInRole), Description = $"User already in role '{role}'."};
        }

        public override IdentityError PasswordTooShort(int length)
        {
            return new IdentityError
                {Code = nameof(PasswordTooShort), Description = $"Passwords must be at least {length} characters."};
        }
        ...
        public override IdentityError PasswordRequiresDigit()
        {
            return new IdentityError
            {
                Code = nameof(PasswordRequiresDigit), Description = "Passwords must have at least one digit ('0'-'9').",
            };
        }
